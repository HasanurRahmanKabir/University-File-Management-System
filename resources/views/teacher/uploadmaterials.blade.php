...
<script>
    ...
    function populateEditModal(id, title, courseId, isActive, folderId) {
        ...
        form.action = TM_MATERIAL_BASE + '/' + id;
        titleInput.value = title || '';

        if (window.editCourseSelect) window.editCourseSelect.setValue(String(courseId));
        else {
            const courseEl = document.getElementById('edit_course_id');
            if (courseEl) courseEl.value = courseId;
        }
        loadFolders(courseId, 'edit_folder_id', folderId ? folderId : null);
        if (window.editPrivacySelect) window.editPrivacySelect.setValue(String(isActive));
        else {
            const privacyEl = document.getElementById('edit_privacy');
            if (privacyEl) privacyEl.value = isActive;
        }
        const fileIn = document.getElementById('fileInEdit');
        const fileText = document.getElementById('fileTextEdit');
        if (fileIn) fileIn.value = '';
        if (fileText) fileText.innerText = 'Click or drag & drop to replace file';
    }
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (editModal) {
            editModal.addEventListener('show.bs.modal', function(ev) {
                const btn = ev.relatedTarget && ev.relatedTarget.closest
                    ? (ev.relatedTarget.closest('.js-edit-material') || ev.relatedTarget)
                    : ev.relatedTarget;
                if (btn && btn.classList && btn.classList.contains('js-edit-material')) {
                    populateEditModal(
                        btn.dataset.id,
                        btn.dataset.title,
                        btn.dataset.courseId,
                        btn.dataset.isActive,
                        btn.dataset.folderId || null
                    );
                }
            });
        }
        ...
        if (renameModal) {
            renameModal.addEventListener('show.bs.modal', function(ev) {
                const btn = ev.relatedTarget && ev.relatedTarget.closest
                    ? (ev.relatedTarget.closest('.js-rename-folder') || ev.relatedTarget)
                    : ev.relatedTarget;
                if (btn && btn.classList && btn.classList.contains('js-rename-folder') && btn.dataset.id) {
                    const form = document.getElementById('renameFolderForm');
                    const nameInput = document.getElementById('rename_folder_name');
                    if (form && nameInput) {
        ...
        if (document.getElementById('edit_course_id')) {
            window.editCourseSelect = new TomSelect('#edit_course_id', tsConfig);
            const si = window.editCourseSelect.dropdown.querySelector('input');
            if (si) si.setAttribute('placeholder', 'Search course...');
            window.editCourseSelect.on('change', function(val) { loadFolders(val, 'edit_folder_id'); });
        }
        ...
        if (document.getElementById('edit_privacy')) {
            window.editPrivacySelect = new TomSelect('#edit_privacy', {
